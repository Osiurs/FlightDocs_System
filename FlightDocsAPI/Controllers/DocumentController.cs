...
public class DocumentController : ControllerBase
{
    ...
    [HttpPost("upload")]
    public async Task<IActionResult> UploadDocument([FromBody] DocumentDto model)
    {
        var document = await _documentService.UploadDocumentAsync(model);
        return CreatedAtAction(nameof(GetDocumentById), new { id = document.FlightID }, document);
    }
